...
fn test_span_coverage_comprehensive() {
    let test_cases = vec![
        ...
        // Functions
        ("let f = fn(x: Number): Number do return x * 2 end", "function"),
        ("f(42)", "function call"),
        ...
        // Match
        ("match x do 1 do return \"one\" end _ do return \"other\" end end", "match expression"),
...
fn test_span_locations_accurate() {
    ...
    assert_eq!(text, "let x = 42", "Span should cover let statement (modulo trailing whitespace)");
    
    // Verify second statement has correct span
    let second_stmt = &program.statements[1];
    let span = second_stmt.span().expect("Second statement should have span");
    assert!(!span.is_empty(), "Span should not be empty");
    let text = span.text(source).trim_end();
    assert_eq!(text, "let y = 100", "Span should cover second let statement (modulo trailing whitespace)");
}

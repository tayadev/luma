//! Expression tests (literals, operators, variables)
...
fn test_string_interpolation_boolean() {
    let source = r#"
-- Test string interpolation with boolean values
let is_ready = true
let status = "Status: ${is_ready}"
status
"#;
    assert_program_output(source, Value::String("Status: true".to_string()));
}

#[test]
fn test_string_interpolation_number() {
    let source = r#"
-- Test string interpolation with number values
let size = 256
let message = "Buffer size: ${size} bytes"
message
"#;
    assert_program_output(source, Value::String("Buffer size: 256 bytes".to_string()));
}
...
// ============================================================================
// Parse Failure Tests (should fail at parse)
// ============================================================================

#[test]
fn test_should_fail_illegal_table_key() {
    let source = r#"
let t = {
  123 = "value"
}
"#;
    assert_program_fails(source, "unexpected");
}

#[test]
fn test_should_fail_invalid_interpolation() {
    let source = r#"let x = "Hello ${name}"#;
    assert_program_fails(source, "unexpected");
}
